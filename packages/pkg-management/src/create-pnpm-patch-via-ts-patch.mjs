...
const program = new commander.Command().addOption(
  new commander.Option('--typescript-version <version>').makeOptionMandatory(),
);
program.parse();
...
await $`pnpm --package='typescript@${opts.typescriptVersion}' --package='ts-patch@3.0.2' dlx ts-patch --dir=${typeScriptTempDir.path} install`;
